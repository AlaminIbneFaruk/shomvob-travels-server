...
const stripe = new Stripe(process.env.STRIPE_TOKEN_SECRET);

const app = express();
const port = process.env.PORT || 9000;

const corsOptions = {
  origin: ["https://assignment-12-server-three-iota.vercel.app/"],
  credentials: true,
};
...
async function connectDB() {
  try {
    ...
    PaymentsC = database.collection("Payments");
...
app.post("/register", async (req, res) => {
  ...
  try {
    const existingUser = await UsersC.findOne({
      $or: [{ username }, { email }],
    });
    if (existingUser) {
...
app.get("/login", async (req, res) => {
  try {
    ...
    res.json({ message: `Name:${user.name} Email: ${user.email}` });
  } catch (error) {
...
app.get("/users", async (req, res) => {
  try {
    const users = await UsersC.find().toArray(); // This works with MongoDB Node.js Driver
    res.json(users);
  } catch (error) {
...
app.get("/packages/random", async (req, res) => {
  try {
    const packages = await PackagesC.aggregate([
      { $sample: { size: 3 } },
    ]).toArray();
    res.json(packages);
  } catch (err) {
...
app.get("/stories/random", async (req, res) => {
  try {
    const stories = await StoriesC.aggregate([
      { $sample: { size: 4 } },
    ]).toArray();
    res.json(stories);
  } catch (error) {
...
app.get("/stories/user", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch user stories", error: error.message });
  }
});
...
app.get("/guides/random", async (req, res) => {
  try {
    const guides = await TourGuidesC.aggregate([
      { $sample: { size: 6 } },
    ]).toArray();
    res.json(guides);
  } catch (error) {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch random guides", error: error.message });
  }
});
// Get guide by ID
app.get("/guides/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch guide", error: error.message });
  }
});
